...
const LoginSignup = () => {
    ...
    <div className="flex flex-col sm:w-[600px] m-auto mt-52 pb-8 border-black border-4">
        ...
        <div className='mt-14 flex flex-col gap-6'>  {/*input*/}
            {action==='Войти'?<div></div>:<div className="flex items-center m-auto w-96 h-20 bg-slate-100">
                <img src={logo} alt="" className="m-0 w-10 h-10 mx-7"/>
                {action==='Войти'?<div></div>:<input type="text" className='h-12 w-96 bg-transparent outline-none text-xl' placeholder="Name"/>}
            </div>}
        ...
        <div className="flex gap-8 ml-16 mb-auto ">
            <div className={action==='Регистрация'?'bg-slate-200 flex justify-center items-center w-56 h-16 text-lg font-bold cursor-pointer':"flex justify-center items-center w-56 h-16 bg-black text-white text-lg font-bold  cursor-pointer"} onClick={() => {setAction('Регистрация')}}>Регистрация</div>  {/*submit*/}
            <div className={action==='Войти'?'bg-slate-200 flex justify-center items-center w-56 h-16 text-lg font-bold  cursor-pointer':" text-white flex justify-center items-center w-56 h-16 bg-black text-lg font-bold  cursor-pointer"} onClick={() => {setAction('Войти')}}>Войти</div>
        </div>
    </div>
  )
}
